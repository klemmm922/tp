...
object Main {
  def main(args: Array[String]) {
    ...
    val groupedDataset = filteredNewsAboutClimate.groupBy("containsWordGlobalWarming").count()
    groupedDataset.show()
    ...
    val tableNews = spark.sql("SELECT DISTINCT media FROM news")
    tableNews.show()
    ...
    newsDatasets.map(_.media).distinct().show()
    ...
    newsDatasets.write.partitionBy("media").parquet("./media-news-parquet")
